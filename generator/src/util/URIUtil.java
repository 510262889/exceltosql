...
public class URIUtil implements Cloneable {
    ...
    private static final Charset __CHARSET = StandardCharsets.UTF_8;
    ...
    public static String addPaths( String p1, String p2 ) {
        ...
        if ( buf.charAt( split - 1 ) == '/' ) {
            ...
            if ( p2.startsWith( URIUtil.SLASH ) ) buf.insert( split, p2 );
            else {
                buf.insert( split, '/' );
                buf.insert( split + 1, p2 );
            }
    ...
    public static String canonicalPath( String path ) {
        ...
        search: while ( end > 0 ) {
            ...
            case 2: // possible single dot
                if ( path.charAt( start + 1 ) != '.' ) break;
                break search;
            case 3: // possible double dot
                if ( path.charAt( start + 1 ) != '.' || path.charAt( start + 2 ) != '.' ) break;
                break search;
            }
        ...
        while ( end > 0 ) {
            ...
            case 2: // possible single dot
                if ( buf.charAt( start + 1 ) != '.' ) {
                    if ( skip > 0 && --skip == 0 ) {
                        ...
                        if ( delStart > 0 && delEnd == buf.length() && buf.charAt( delEnd - 1 ) == '.' ) delStart++;
                    }
                    break;
                }
                ...
                if ( delEnd < 0 ) delEnd = end;
                delStart = start;
                if ( delStart < 0 || delStart == 0 && buf.charAt( delStart ) == '/' ) {
                    delStart++;
                    if ( delEnd < buf.length() && buf.charAt( delEnd ) == '/' ) delEnd++;
                    break;
                }
                if ( end == buf.length() ) delStart++;
            ...
            default:
                if ( skip > 0 && --skip == 0 ) {
                    delStart = start >= 0 ? start : 0;
                    if ( delEnd == buf.length() && buf.charAt( delEnd - 1 ) == '.' ) delStart++;
                }
            }
        ...
        // Do the delete
        if ( delEnd >= 0 ) buf.delete( delStart, delEnd );
