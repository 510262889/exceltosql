...
class JarFileResource extends JarResource {
    ...
    public synchronized void release() {
        ...
        _entry = null;
        _file = null;
        // if the jvm is not doing url caching, then the JarFiles will not be
        // cached either,
        // and so they are safe to close
        if ( !getUseCaches() ) {
            if ( _jarFile != null ) {
                try {
                    System.out.println( "Closing JarFile " + _jarFile.getName() );
                    _jarFile.close();
    ...
    public synchronized String[] list() {
        if ( isDirectory() && _list == null ) {
            ...
            try {
                ...
                // Sun's JarURLConnection impl for jar: protocol will close a
                // JarFile in its connect() method if
                // useCaches == false (eg someone called URLConnection with
                // defaultUseCaches==true).
                // As their sun.net.www.protocol.jar package caches JarFiles
                // and/or connections, we can wind up in
                // the situation where the JarFile we have remembered in our
                // _jarFile member has actually been closed
                // by other code.
                // So, do one retry to drop a connection and get a fresh JarFile
                System.out.println( "Retrying list:" + e );
                e.printStackTrace();
                release();
                list = listEntries();
            }
